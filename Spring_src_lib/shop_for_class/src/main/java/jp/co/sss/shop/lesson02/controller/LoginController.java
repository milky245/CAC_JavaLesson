...
/**
 * lesson02のログイン処理を担当するコントローラクラスです。
 *
 * ログイン画面表示、ログイン実行、ログアウト、ユーザー詳細画面表示を行います。
 */
@Controller
public class LoginController {

	/** ユーザー情報を検索するためのRepositoryです。 */
	@Autowired
	...
	/**
	 * ログインIDとパスワードをチェックしてログイン処理を行います。
	 *
	 * 入力チェックエラーまたは認証エラーの場合は、ログイン画面に戻します。
	 *
	 * @param loginForm ログイン入力フォーム
	 * @param result 入力チェック結果
	 * @param session ログインユーザーを保存するセッション
	 * @param model 画面へ渡すデータを格納するModel
	 * @return ログイン成功時はコメント一覧画面へリダイレクト、失敗時はログイン画面
	 */
